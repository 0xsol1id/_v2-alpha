import Link from "next/link";
import React from "react";
import { DiscordLogo, TwitterLogo, GithubLogo, GameLogo } from "components"

export const Footer: React.FC = ({ }) => {
    return (
        <div>
            <div className="fixed bottom-0 left-0 right-0 bg-gray-900 p-2 lg:p-1 flex justify-between lg:hidden items-center">
                <button className="btn btn-sm bg-green-500 mx-1">
                    <Link href="/mint">
                        <img src="./button/buy_button.png" className="h-8"/>
                    </Link>
                </button>
                <h1 className="font-pixel text-xs">DegenBags V0.1_ALPHA</h1>
            </div>
        </div>
    )
}