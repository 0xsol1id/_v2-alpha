    ...
    return (
        <div>
            {!iscreating && !isNaN(quantity) && !isNaN(decimals) &&
                <button className="btn btn-primary uppercase mt-2 w-64" onClick={() => { if (publicKey) createSPLToken(publicKey, wallet, connection, quantity, decimals, isChecked, tokenName, symbol, metadataURL, description, file, metadataMethod, setIscreating, setTokenAddresss, setSignature, setError); else setVisible(true) }}>Create token</button>
            }

            {!iscreating && (isNaN(quantity) || isNaN(decimals)) &&
                <div>
                    <button className="btn btn-primary uppercase mt-2 w-64" disabled>Create token</button>
                    <p className="mt-5">❌ Please fill in both fields with a number</p>
                </div>
            }

            {iscreating &&
                <button className="btn btn-primary uppercase mt-2 w-64">
