    ...
    return (
        <div>
            {!isSelected && delegated == true &&
                <button className="btn bg-[#55268e] hover:bg-[#3d1b66] uppercase mb-2 sm:mb-4 sm:mr-1" onClick={() => { setIsSelected(true); toRevoke.push(tokenMintAddress) }}>✔️</button>
            }
            {isSelected && delegated == true &&
                <button className="btn bg-[#3d1b66] hover:bg-[#55268e] uppercase mb-2 sm:mb-4 sm:mr-1" onClick={() => { setIsSelected(false); toRevoke.splice(toRevoke.indexOf(tokenMintAddress), 1) }}>❌</button>
            }
